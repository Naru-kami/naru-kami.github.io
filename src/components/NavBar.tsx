...
function Links({ location, orientation }: { location: Location, orientation?: "horizontal" | "vertical" }) {
  const verHeight = useMemo(() => orientation === "vertical" ? '36px' : '24px', [orientation]);
  const backgroundColor = useMemo(() => orientation === "vertical" ? '#307ac3' : 'transparent', [orientation]);
  const backgroundImage = useMemo(() => orientation === "vertical" ? 'linear-gradient(-90deg, #0000, color-mix(in oklch, #307ac3 50%, #0000))' : 'linear-gradient(#0000, color-mix(in oklch, #307ac3 30%, #0000))', [orientation]);
  const m = useMemo(() => orientation === "vertical" ? '0 8px 0 0' : '0', [orientation]);
  const borderRadius = useMemo(() => orientation === "vertical" ? '0 8px 8px 0' : '8px 8px 0 0', [orientation]);
  const pathname = location.pathname.split('/')[1];

  return (
    <Tabs
      value={pathname?.includes("Wishing") ? "Wishing" : pathname?.includes("Artifacts") ? "Artifacts" : "/"}
