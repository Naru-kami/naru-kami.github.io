...
export default function CurrencySwitcher(props: ButtonProps) {
  ...
  return <Tooltip arrow placement="top" title={<Typography variant="body2">{usePrimogem ? "Using Primogems" : "Using Fates"}</Typography>}>
    <Button onClick={handleClick} variant='contained' {...props} sx={{ display: mode === 'distribution' ? 'inline-flex' : 'none', minWidth: 0, p: 0.5, backgroundColor: '#529ef8', backgroundImage: 'linear-gradient(135deg, #f583cc, transparent)', ...props.sx }}>
